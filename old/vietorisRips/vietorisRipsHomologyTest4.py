...
def VietorisRips(data, epsilon):
    ...
    for i in range(len(dataCopy)):
        data.remove(dataCopy[i])
        mergeSortDist(data, dataCopy[i], dim)
        j = 0
        while True:
            if dist(dataCopy[i], data[j], dim) >= epsilon or j >= len(data)-1:
                break
            else:
                C[1].append([dataCopy[i],data[j]])
                k = 0
                while True:
                    if dist(dataCopy[i], data[k], dim) >= epsilon or k >= len(data)-1:
                        break
                    elif dist(data[j], data[k], dim) < epsilon:
                        C[2].append([dataCopy[i],data[j],data[k]])
                        k += 1
                    else:
                        k += 1
                j += 1        

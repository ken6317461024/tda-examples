                    ...
                    if dist(data[0], data[k], dim) >= epsilon:
                        ...
                        while l < L:
                            if dist(data[0], data[l], dim) >= epsilon:
                                ...
                                m = l+1
                                while m < L:
                                    if dist(data[0], data[m], dim) >= epsilon:
                                        break
                                    elif dist(data[j], data[m], dim) >= epsilon:
                                        break
                                    elif dist(data[k], data[m], dim) >= epsilon:
                                        break
                                    elif dist(data[l], data[m], dim) >= epsilon:
                                        break
                                    else:
                                        C[4].append([data[0],data[j],data[k],data[l],data[m]])
                                    m += 1
                                l += 1
                        k += 1
                j += 1   
...
for i in range(len(coordinates)):
    ...
    coordinates.append([coordinates[i][0], coordinates[i][1], -1000*math.cos((Z)/2)])
    coordinates.append([coordinates[i][0], coordinates[i][1], 2000+1000*math.cos((Z)/2)])
    coordinates.append([coordinates[i][0], coordinates[i][1], 2000-1000*math.cos((Z)/2)])    
...
for i in range(N):
    centersList.append([round(centers[i][0],2),round(centers[i][1],2),round(centers[i][2],2)])
